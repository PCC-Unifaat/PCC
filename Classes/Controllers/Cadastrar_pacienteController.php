		...
		function index(){
			...
			if(isset($_POST['acao'])){
				...
				if(empty($nome) || empty($cpf) || empty($nascimetno))
					...
					//Cadastrar
                    $sql = \Classes\MySql::conectar()->prepare("INSERT INTO `$tabela` SET agente_id = ?, nome = ?, cpf = ?, nascimento = ?, prontuario = ?");
                    $sql->execute([$_SESSION['id'],$nome,$cpf,$nascimetno,$prontuario]);
